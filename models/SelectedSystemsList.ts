...
export class SelectedSystemsList implements SelectedSystemsListInterface {
  ...
  _selectedSystems = new Map<SystemIdType, SelectedSystemLogType>() as SelectedSystemMapType;
  ...
  selectSystem(system: SystemModelInterface) : void {
    if (! this._selectedSystems.has(system.id)) {
  ...
  clearSelectedSystems() {
    this._selectedSystems.clear();
  }

  systemIsSelected(system: SystemModelInterface) {
    return this._selectedSystems.has(system.id);
  }
}
