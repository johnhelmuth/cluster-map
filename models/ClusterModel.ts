...
import type {SystemIdType, SystemModelDataType, SystemModelInterface} from '@/types/SystemTypes';
...
import {isClusterModelDataType} from "@/types/ClusterTypes";
...
export class ClusterModel implements ClusterModelInterface {
  ...
  constructor(data?: ClusterModelDataType) {
    ...
    this.straits = [];
    if (data && isClusterModelDataType(data)) {
  ...
  getStraitsBySystem(system: SystemModel): Array<StraitModelInterface> {
  ...
  importStraits(data: ClusterModelDataType) {
    if (data.straits) {
      for (const straitData of data.straits ?? []) {
        const systemA = this.getSystemById(straitData.systems[0]);
        if (systemA) {
