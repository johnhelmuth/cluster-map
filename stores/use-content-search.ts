...
export async function useContentSearch() {
  const {data} = await useAsyncData('search',
    async () => {
      const contentData = await queryCollectionSearchSections('content')
      const campaignsData = await queryCollectionSearchSections('campaigns')
      const newsData = await queryCollectionSearchSections('news')
      return [
        ...contentData,
        ...campaignsData,
        ...newsData
